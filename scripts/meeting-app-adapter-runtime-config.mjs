...
const includeConfigs = args.get('include-configs') === 'true';
...
function configFile(config = {}) {
  return resolve(outDir, `${config.adapter_key || config.platform || 'adapter'}.json`);
}
...
const report = {
  type: 'meeting_app_adapter_runtime_config_report',
  ...
  capture_ready_count: matrix.capture_ready_count,
  mutation_ready_count: matrix.mutation_ready_count,
  ...
  rows: matrix.rows.map((row) => ({
    ...row,
    config_file: outDir ? configFile(row) : undefined,
  })),
  matrix: includeConfigs ? matrix : {
    ...matrix,
    configs: undefined,
  },
...
if (jsonOutput) {
  ...
  console.log(`meeting_app_adapter_runtime_config_report | ok=${boolLabel(report.ok)} | configs=${report.config_count} | accepted=${report.accepted_count} | built_in=${report.built_in_count} | custom=${report.custom_count} | capture_ready=${report.capture_ready_count} | mutation_ready=${report.mutation_ready_count} | content_script_ready=${report.content_script_ready_count} | written=${report.written_files.length}`);
  for (const row of report.rows) {
    console.log(`${row.adapter_key}: accepted=${boolLabel(row.accepted)} source=${row.source} capture=${boolLabel(row.capture_options_ready)} mutation=${boolLabel(row.mutation_observer_ready)} content_script=${boolLabel(row.content_script_ready)} controls=${row.control_selector_count} participants=${row.participant_selector_count} mutation_selectors=${row.mutation_track_selector_count} next=${row.first_next_action ?? 'none'} config=${row.config_file ? basename(row.config_file) : 'n/a'}`);
  }
