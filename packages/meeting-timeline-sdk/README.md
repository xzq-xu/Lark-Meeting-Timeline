...
如果要给另一个项目一个更完整的“可改造骨架”，用 `platform-host-integration` 生成 host scaffold。它会输出 `package.json`、timeline client、host wrapper、framework-neutral HTTP route、strategy/handoff/readiness/runtime-bundle/extension-plan/integration-runtime 脚本和 README。生成的 host wrapper 会先创建 `createMeetingPlatformIntegrationRuntime()`，因此下游项目可以优先接统一 runtime，再按需下钻到 strategy、runtime bundle 或 extension plan：
