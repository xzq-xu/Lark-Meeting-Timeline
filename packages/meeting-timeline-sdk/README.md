...
`host-install-checklist.json`、`connector-adoption-index.json`、`connector-adapter-matrix.json` 和 `host-adapter-configs/{platform}.json` 都会带 `adapter_preflight` / `evidence_contract.adapter_preflight`：浏览器类 surface 会要求 live DOM/current-window evidence，native detector 会要求窗口、进程、Accessibility 或音频通话状态；两者都会明确 `url_only_status=needs_live_page_evidence`，保证 URL-only 识别不会被误当作可实时插入标注。同一批产物也会带 `local_observer_contract`，把 Google Meet/Teams/Webex/Lark 的 DOM 采样、Zoom 等 native window 采样、speaker 稳定化阈值、participant 去抖、`captured_at_ms` 时间戳规则和 provider/transcript 不阻塞实时标注的规则交给宿主直接执行。
