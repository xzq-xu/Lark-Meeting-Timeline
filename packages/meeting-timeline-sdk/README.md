...
如果宿主项目不是所有 surface 都能做，可以把能力直接传进决策层，让 SDK 自动选可用路径。例如只装浏览器扩展时传 `availableSurfaces: ['browser_extension']`，Google Meet / Zoom / Teams 会统一落到 browser observer；只具备 native/accessibility agent 时传 `hostCapabilities: { nativeDetector: true }`，Google Meet 也会改选 `native_detector`；如果只传 `availableSurfaces: ['provider_reconcile']`，SDK 会保留 provider reconcile 路径用于会后/生产补齐，但 `accepted=false`、`status='needs_local_surface_for_realtime_axis'`，不会把 provider webhook 误判成低延迟实时轴。
