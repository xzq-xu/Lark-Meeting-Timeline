...
function browserLikeSurface(surface) {
  const normalized = normalizeKey(surface);
  return normalized === 'browser_extension'
    || normalized === 'webview_preload'
    || normalized === 'electron_webview';
}

function nativeLikeSurface(surface) {
  const normalized = normalizeKey(surface);
  return normalized === 'native_detector'
    || normalized === 'native_host'
    || normalized === 'desktop_observer';
}
...
function runtimeProfileForRow(row = {}) {
  const platform = normalizeKey(row.platform);
  if (!platform) return {};
  try {
    return buildMeetingPlatformRuntimeProfile(platform);
  } catch {
    return {};
  }
}

function localObserverInputContract(surface, domProfile = {}) {
  const browserLike = browserLikeSurface(surface);
  const nativeLike = nativeLikeSurface(surface);
  if (browserLike) {
    return {
      kind: 'browser_live_dom',
      required_inputs: ['current_window_document_or_live_dom_snapshot', 'candidate_tabs_or_windows'],
      optional_inputs: ['shadow_dom_snapshot', 'active_speaker_dom_signal', 'participant_roster_dom_signal'],
      capture_profile: domProfile.platform,
      selector_groups: {
        controls: domProfile.controlSelectors ?? [],
        participants: domProfile.participantSelectors ?? [],
        texts: domProfile.textSelectors ?? [],
      },
    };
  }
  if (nativeLike) {
    return {
      kind: 'native_window_or_process',
      required_inputs: ['candidate_windows', 'process_or_bundle_identity', 'call_active_state'],
      optional_inputs: ['accessibility_tree', 'audio_activity_sample', 'active_speaker_accessibility_signal'],
      native_fields: ['process.name', 'bundle.id', 'window.title', 'window.active', 'accessibility.call_active', 'audio.active_speaker'],
    };
  }
  return {
    kind: 'host_surface_snapshot',
    required_inputs: ['candidate_surface_snapshot'],
    optional_inputs: ['active_speaker_signal', 'participant_roster_signal'],
  };
}

function localObserverContract(row = {}) {
  const platform = normalizeKey(row.platform);
  const surface = normalizeKey(row.selected_surface);
  const profile = runtimeProfileForRow({ ...row, platform });
  const domProfile = browserLikeSurface(surface) ? meetingAppDomCaptureProfile(platform) ?? {} : {};
  const speakerFilter = profile.speaker_markers?.filter ?? {};
  return compactObject({
    required: true,
    platform,
    selected_surface: surface,
    observer_mode: browserLikeSurface(surface)
      ? 'browser_dom_observer'
      : nativeLikeSurface(surface)
        ? 'native_window_observer'
        : 'host_surface_observer',
    axis: {
      start_create_on: profile.axis?.start?.create_on ?? 'local_observer_active_meeting_detected',
      end_create_on: profile.axis?.end?.create_on ?? 'local_observer_meeting_inactive_or_manual_stop',
      end_fallbacks: profile.axis?.end?.fallbacks ?? [],
      provider_reconcile_required_for_realtime: false,
    },
    sampling: {
      candidate_scan_interval_ms: 1_000,
      active_snapshot_interval_ms: browserLikeSurface(surface) ? 1_000 : 750,
      speaker_sample_interval_ms: speakerFilter.sample_interval_ms,
      participant_snapshot_interval_ms: 2_000,
      max_snapshot_age_ms_before_insert: 2_500,
    },
    filters: {
      speaker: speakerFilter,
      participant: {
        duplicate_window_ms: 1_500,
        leave_stable_ms: 3_000,
      },
    },
    input_contract: localObserverInputContract(surface, domProfile),
    output_events: [
      'observe_platform_candidates',
      'observe_meeting_app',
      'speaker_track',
...
function platformOnboardingByPlatform(pkg = {}) {
  const checklists = pkg.platform_onboarding?.matrix?.checklists
    ?? pkg.platform_onboarding?.checklists
    ?? [];
  return new Map(asArray(checklists).map((checklist) => [normalizeKey(checklist.platform), checklist]));
}

function summarizePlatformOnboardingContract(checklist = {}) {
  if (!checklist || typeof checklist !== 'object' || !checklist.schema) return undefined;
  const installStep = asArray(checklist.implementation_sequence)
    .find((step) => step.group === 'local_observer_install');
  return compactObject({
    schema: checklist.schema,
    target: checklist.target,
    accepted: checklist.accepted === true,
    adapter_status: checklist.adapter_status,
    recommended_first_surface: checklist.recommended_first_surface,
    install_step: installStep?.id,
    runtime_event_contract: checklist.runtime_event_contract,
    implementation_sequence: checklist.implementation_sequence,
    sdk_entrypoints: checklist.sdk_entrypoints,
    evidence_collection_plan: checklist.evidence_collection_plan,
    summary: checklist.summary,
    next_actions: checklist.next_actions,
  });
}

function summarizePlatformOnboarding(pkg = {}) {
  const matrix = pkg.platform_onboarding?.matrix;
  if (!matrix) return undefined;
  return compactObject({
    schema: matrix.schema,
    target: matrix.target,
    accepted: pkg.platform_onboarding?.accepted === true || matrix.blocked_count === 0,
    accepted_count: matrix.accepted_count,
    blocked_count: matrix.blocked_count,
    platform_count: matrix.platform_count,
    command: pkg.platform_onboarding?.command,
    sdk_method: pkg.platform_onboarding?.sdk_method,
    rows: matrix.rows,
  });
}
...
export function buildMeetingAppTimelineConnectorPackageAcceptanceReport(pkg = {}, options = {}) {
  ...
  if (pkg.platform_onboarding && pkg.platform_onboarding.accepted !== true) {
    addIssue(issues, 'platform_onboarding_not_accepted', 'Platform onboarding checklist matrix must be accepted for connector handoff', {
      target: pkg.platform_onboarding.target,
      blocked_count: pkg.platform_onboarding.blocked_count,
    });
  }
  if (pkg.platform_onboarding?.matrix && (pkg.platform_onboarding.matrix.platform_count ?? 0) < platforms.length) {
    addIssue(issues, 'platform_onboarding_coverage_incomplete', 'Platform onboarding matrix must cover every selected platform', {
      platform_onboarding_platform_count: pkg.platform_onboarding.matrix.platform_count ?? 0,
      platform_count: platforms.length,
    });
  }
...
export function buildMeetingAppTimelineConnectorHostInstallChecklist(pkg = {}, options = {}) {
  ...
  const onboardingByPlatform = platformOnboardingByPlatform(pkg);
  const rows = acceptance.platforms.map((platform) => {
    ...
    const onboarding = summarizePlatformOnboardingContract(onboardingByPlatform.get(platform));
    ...
    const row = compactObject({
      ...
      required_host_steps: [
        ...
        'apply_local_observer_sampling_and_filter_contract',
  ...
  return compactObject({
    ...
    platform_onboarding: summarizePlatformOnboarding(pkg),
...
export function buildMeetingAppTimelineConnectorHostInstallChecklistAcceptanceReport(checklistOrPackage = {}, options = {}) {
  ...
  if (checklist.platform_onboarding && checklist.platform_onboarding.accepted !== true) {
    addIssue(issues, 'platform_onboarding_not_accepted', 'Platform onboarding summary must be accepted', {
      target: checklist.platform_onboarding.target,
      blocked_count: checklist.platform_onboarding.blocked_count,
    });
  }
  ...
  for (const row of rows) {
    ...
    if (!requiredSteps.includes('apply_local_observer_sampling_and_filter_contract')) {
      addIssue(issues, 'row_missing_local_observer_contract_step', 'Checklist row must require applying local observer sampling and filter contract', { platform });
    }
    ...
    if (row.local_observer_contract?.required !== true) {
      addIssue(issues, 'row_missing_local_observer_contract', 'Checklist row must include local observer contract', { platform });
    }
    ...
    if (row.onboarding_contract?.schema !== 'meeting_platform_adapter_acceptance_checklist') {
      addIssue(issues, 'row_missing_onboarding_contract', 'Checklist row must include platform onboarding contract', { platform });
    }
    if (row.onboarding_contract?.runtime_event_contract?.timestamp_field !== 'captured_at_ms') {
      addIssue(issues, 'row_onboarding_missing_captured_at_ms', 'Platform onboarding contract must preserve captured_at_ms', { platform });
    }
    if (row.onboarding_contract?.runtime_event_contract?.observe_before_insert_required !== true) {
      addIssue(issues, 'row_onboarding_missing_observe_before_insert', 'Platform onboarding contract must require observe before insert', { platform });
    }
    if (row.onboarding_contract?.runtime_event_contract?.speaker_position_markers?.transcript_text_required !== false) {
      addIssue(issues, 'row_onboarding_requires_transcript_for_speaker_positions', 'Speaker position markers must not require transcript text', { platform });
    }
    const onboardingSteps = asArray(row.onboarding_contract?.implementation_sequence).map((step) => step.id);
    if (!onboardingSteps.includes('observe_platform_candidates_before_insert')) {
      addIssue(issues, 'row_onboarding_missing_observe_step', 'Platform onboarding sequence must include observe_platform_candidates_before_insert', { platform });
    }
    if (!onboardingSteps.includes('insert_annotation_with_captured_at_ms')) {
      addIssue(issues, 'row_onboarding_missing_insert_step', 'Platform onboarding sequence must include insert_annotation_with_captured_at_ms', { platform });
    }
  ...
  return compactObject({
    ...
    rows: rows.map((row) => compactObject({
      ...
      onboarding_target: row.onboarding_contract?.target,
      onboarding_install_step: row.onboarding_contract?.install_step,
      onboarding_timestamp_field: row.onboarding_contract?.runtime_event_contract?.timestamp_field,
    })),
    next_actions: issues.length > 0
      ? unique([
        ...issues.map((issue) => `fix_${issue.code}`),
...
export function buildMeetingAppTimelineConnectorAdoptionIndex(checklistOrPackage = {}, options = {}) {
  const checklist = checklistOrPackage?.schema === MEETING_APP_TIMELINE_CONNECTOR_HOST_INSTALL_CHECKLIST_SCHEMA
    ? checklistOrPackage
    : buildMeetingAppTimelineConnectorHostInstallChecklist(checklistOrPackage, options);
  const fieldIntakeIndex = firstOption(options, ['fieldIntakeIndex', 'field_intake_index'])
    ?? buildMeetingAppTimelineConnectorFieldIntakeIndex(checklist, options);
  const bridgeHandoff = firstOption(options, ['bridgeHandoff', 'bridge_handoff'])
    ?? buildMeetingAppTimelineConnectorBridgeHandoff(checklist, options);
  const bridgeAcceptance = firstOption(options, [
    'bridgeHandoffAcceptance',
    'bridge_handoff_acceptance',
    'bridgeHandoffAcceptanceReport',
    'bridge_handoff_acceptance_report',
  ]) ?? buildMeetingAppTimelineConnectorBridgeHandoffAcceptanceReport(bridgeHandoff, options);
  const smokePlan = firstOption(options, ['smokePlan', 'smoke_plan'])
    ?? buildMeetingAppTimelineConnectorSmokePlan(checklist, options);
  const smokeAcceptance = firstOption(options, [
    'smokePlanAcceptance',
    'smoke_plan_acceptance',
    'smokePlanAcceptanceReport',
    'smoke_plan_acceptance_report',
...
export function buildMeetingAppTimelineConnectorBridgeHandoff(pkgOrChecklist = {}, options = {}) {
  ...
  const consumerHandoff = firstOption(options, ['consumerHandoff', 'consumer_handoff'])
    ?? buildMeetingPlatformConsumerHandoff({
    ...options,
    baseUrl,
    platforms,
...
export function buildMeetingAppTimelineConnectorReleaseGate(checklistOrPackage = {}, options = {}) {
  ...
  const suppliedPackageAcceptance = firstNonEmpty(
    firstOption(options, [
      'packageAcceptance',
      'package_acceptance',
      'packageAcceptanceReport',
      'package_acceptance_report',
    ]),
  ...
  const packageAcceptance = suppliedPackageAcceptance
  ...
  const hostInstallChecklistAcceptance = firstOption(options, [
    'hostInstallChecklistAcceptance',
    'host_install_checklist_acceptance',
    'hostInstallChecklistAcceptanceReport',
    'host_install_checklist_acceptance_report',
  ...
  const bridgeHandoffAcceptance = firstOption(options, [
    'bridgeHandoffAcceptance',
    'bridge_handoff_acceptance',
    'bridgeHandoffAcceptanceReport',
    'bridge_handoff_acceptance_report',
  ...
  const smokePlanAcceptance = firstOption(options, [
    'smokePlanAcceptance',
    'smoke_plan_acceptance',
    'smokePlanAcceptanceReport',
    'smoke_plan_acceptance_report',
  ]) ?? buildMeetingAppTimelineConnectorSmokePlanAcceptanceReport(smokePlan, options);
  const adoptionIndex = firstOption(options, ['adoptionIndex', 'adoption_index'])
    ?? buildMeetingAppTimelineConnectorAdoptionIndex(hostInstallChecklist, {
      ...options,
      fieldIntakeIndex,
      bridgeHandoff,
      bridgeHandoffAcceptance,
  ...
  const bridgeSmokeReport = firstOption(options, ['bridgeSmokeReport', 'bridge_smoke_report']);
  const smokeRunReport = firstOption(options, ['smokeRunReport', 'smoke_run_report']);
  const hostAdapterBootstrapPlanMatrixAcceptance = firstOption(options, [
    'hostAdapterBootstrapPlanMatrixAcceptance',
    'host_adapter_bootstrap_plan_matrix_acceptance',
    'hostAdapterBootstrapPlanMatrixAcceptanceReport',
    'host_adapter_bootstrap_plan_matrix_acceptance_report',
    'hostBootstrapPlanMatrixAcceptance',
    'host_bootstrap_plan_matrix_acceptance',
    'hostBootstrapAcceptanceReport',
    'host_bootstrap_acceptance_report',
  ]);
...
export function buildMeetingAppTimelineConnectorPlatformRoadmap(checklistOrPackage = {}, options = {}) {
  const hostInstallChecklist = checklistOrPackage.schema === MEETING_APP_TIMELINE_CONNECTOR_HOST_INSTALL_CHECKLIST_SCHEMA
    ? checklistOrPackage
    : buildMeetingAppTimelineConnectorHostInstallChecklist(checklistOrPackage, options);
  const releaseGate = firstOption(options, ['releaseGate', 'release_gate'])
...
export function buildMeetingAppTimelineConnectorAdapterMatrix(checklistOrPackage = {}, options = {}) {
  ...
  const fieldIntakeIndex = firstOption(options, ['fieldIntakeIndex', 'field_intake_index'])
    ?? buildMeetingAppTimelineConnectorFieldIntakeIndex(hostInstallChecklist, options);
  const bridgeHandoff = firstOption(options, ['bridgeHandoff', 'bridge_handoff'])
    ?? buildMeetingAppTimelineConnectorBridgeHandoff(hostInstallChecklist, {
      ...options,
      consumerHandoff,
    });
  const bridgeHandoffAcceptance = firstOption(options, [
  ...
  const releaseGate = firstOption(options, ['releaseGate', 'release_gate'])
    ?? buildMeetingAppTimelineConnectorReleaseGate(hostInstallChecklist, {
      ...options,
      fieldIntakeIndex,
      bridgeHandoff,
      bridgeHandoffAcceptance,
  ...
  const roadmap = firstOption(options, ['platformRoadmap', 'platform_roadmap'])
  ...
  const rows = (hostInstallChecklist.rows ?? []).map((row) => {
    ...
    return compactObject({
      ...
      evidence_contract: {
        ...
        adapter_preflight: row.adapter_preflight,
        platform_onboarding: row.onboarding_contract,
  ...
  return compactObject({
    ...
    source_schemas: {
      ...
      platform_onboarding: hostInstallChecklist.platform_onboarding?.schema,
    },
    files_to_read_first: [
      'connector-adapter-matrix.json',
      'provider-replay-matrix.json',
...
export function buildMeetingAppTimelineConnectorAdapterMatrixAcceptanceReport(matrixOrChecklistOrPackage = {}, options = {}) {
  ...
  for (const row of rows) {
    ...
    if (row.local_observer_contract?.required !== true) {
      addIssue(issues, 'row_missing_local_observer_contract', 'Adapter matrix row must include local observer contract', { platform });
    }
    if (row.local_observer_contract?.realtime_rules?.timestamp_field !== 'captured_at_ms') {
      addIssue(issues, 'row_local_observer_missing_captured_at_ms', 'Local observer contract must preserve captured_at_ms', { platform });
    }
    ...
    if (row.onboarding_contract?.schema !== 'meeting_platform_adapter_acceptance_checklist') {
      addIssue(issues, 'row_missing_onboarding_contract', 'Adapter matrix row must include platform onboarding contract', { platform });
    }
    if (row.onboarding_contract?.runtime_event_contract?.timestamp_field !== 'captured_at_ms') {
      addIssue(issues, 'row_onboarding_missing_captured_at_ms', 'Platform onboarding contract must preserve captured_at_ms', { platform });
    }
...
function hostAdapterConfigIssues(config = {}) {
  ...
  if (config.onboarding_contract?.schema !== 'meeting_platform_adapter_acceptance_checklist') {
    addIssue(issues, 'missing_onboarding_contract', 'Host adapter config must include platform onboarding contract', { platform: config.platform });
  }
  if (config.onboarding_contract?.runtime_event_contract?.timestamp_field !== 'captured_at_ms') {
    addIssue(issues, 'onboarding_missing_captured_at_ms', 'Host adapter onboarding contract must preserve captured_at_ms', { platform: config.platform });
  }
  if (config.onboarding_contract?.runtime_event_contract?.observe_before_insert_required !== true) {
    addIssue(issues, 'onboarding_missing_observe_before_insert', 'Host adapter onboarding contract must require observe before insert', { platform: config.platform });
  }
  if (config.local_observer_contract?.required !== true) {
    addIssue(issues, 'missing_local_observer_contract', 'Host adapter config must include local observer contract', { platform: config.platform });
  }
  if (config.local_observer_contract?.realtime_rules?.timestamp_field !== 'captured_at_ms') {
    addIssue(issues, 'local_observer_missing_captured_at_ms', 'Host adapter local observer contract must preserve captured_at_ms', { platform: config.platform });
  }
