...
export function buildMeetingPlatformRuntimeBundle(platform, options = {}) {
  ...
  const observerSchedulerConfig = observerPlan
    ? buildMeetingAppObserverSchedulerConfig(observerPlan, options)
    : null;
  ...
  return compactObject({
    ...
    modules: {
      ...
      observer_scheduler: '@ai-annotation/meeting-timeline-sdk/adapters/meeting-app-observer-scheduler',
    ...
    runtime: {
      ...
      observer_scheduler: observerSchedulerConfig ? {
        module: '@ai-annotation/meeting-timeline-sdk/adapters/meeting-app-observer-scheduler',
        create_function: 'createMeetingAppObserverScheduler',
        config: observerSchedulerConfig,
      } : null,
      runtime_host: observerSchedulerConfig ? {
        module: '@ai-annotation/meeting-timeline-sdk/adapters/meeting-platform-runtime-host',
        create_function: 'createMeetingPlatformRuntimeHost',
        config_function: 'buildMeetingPlatformRuntimeHostConfig',
        driver: 'observer_scheduler_plus_host_change_observer',
      } : null,
    },
    messaging: {
...
export function buildMeetingPlatformRuntimeBundleMatrix(options = {}) {
  ...
  return {
    ...
    rows: bundles.map((bundle) => ({
      ...
      adapter_recommended_mode: bundle.adapter_route?.recommended_mode,
      adapter_first_route: bundle.adapter_route?.routes?.[0]?.route,
