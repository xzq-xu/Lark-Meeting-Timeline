...
export function buildMeetingAppExtensionContentScriptSource(options = {}) {
  ...
  return [
    ...
    '}',
    '',
    'function lifecycleEndedEvidenceRecord(meeting, eventName, capturedAtMs) {',
    '  const platform = inferPlatform();',
    ...
    '    schema_version: 1,',
    '    source: "meeting_app_extension_lifecycle_evidence",',
    '    observedAtMs: capturedAtMs,',
    '    url,',
    '    title,',
    '    inMeeting: false,',
    ...
    '    semanticSignals: [],',
    '    semanticSignalTypes: [],',
    '    page: { url, title, inMeeting: false, interaction: {}, semanticSignals: [], semanticSignalTypes: [] },',
    '    dom: { url, title, inMeeting: false, interaction: {}, semanticSignals: [], semanticSignalTypes: [] },',
    '    lifecycle: { event: eventName, meeting_was_active: true },',
    '  };',
    '  return {',
    '    schema: "meeting_app_snapshot_record",',
    '    schema_version: 1,',
    '    id: platform + ":ended:" + capturedAtMs,',
    '    platform,',
    '    phase: "ended",',
    '    label: "meeting_ended",',
    '    captured_at_ms: capturedAtMs,',
    ...
    '}',
    '',
    'function createLifecycleEndReporter(bridge) {',
    '  let sent = false;',
    '  const report = (eventName = "pagehide", detectorSource = "meeting_app_extension_pagehide", capturedAtMs = Date.now()) => {',
    '    if (sent) return false;',
    '    const meeting = bridge.observer?.getState?.()?.sessionState?.activeMeeting;',
    '    if (!meeting?.meeting_id) return false;',
    '    sent = true;',
    '    callTimeline("endMeeting", {',
    '      ...meeting,',
    '      end_time_ms: capturedAtMs,',
    '      captured_at_ms: capturedAtMs,',
    '      detector_source: detectorSource,',
    '      observer_surface: "browser_extension",',
    '      suppress_auto_annotations: true,',
    '      meeting_app_record: lifecycleEndedEvidenceRecord(meeting, eventName, capturedAtMs),',
    '    }).catch(() => {});',
    '    return true;',
    '  };',
    '  return { report, wasSent: () => sent };',
    '}',
    '',
    'function bindLifecycleEndCapture(endReporter) {',
    '  const listener = (event = {}) => {',
    '    endReporter.report(event.type || "pagehide", "meeting_app_extension_pagehide");',
    '  };',
    '  globalThis.addEventListener?.("pagehide", listener, { capture: true });',
    '  globalThis.addEventListener?.("beforeunload", listener, { capture: true });',
    '  return { installed: true, events: ["pagehide", "beforeunload"], listener };',
    '}',
    '',
    ...
    '  if (/\\bleave call\\b|\\bleave meeting\\b|\\bend call\\b|\\bend meeting\\b|\\bhang up\\b|离开通话|离开会议|结束通话|结束会议|挂断|(?:^|\\s)(?:离开|退出)(?:\\s|$)/i.test(label)) return "leave";',
    '  return null;',
    '}',
    '',
    'function bindOperatorReferenceCapture(bridge, endReporter) {',
    ...
    '    }).catch(() => {});',
    '    if (action === "leave") {',
    '      endReporter?.report?.("operator_leave", "meeting_app_extension_operator_leave", atMs);',
    '    }',
    '  };',
    ...
    '  captureOptions: { includeSameOriginFrames: true },',
    ...
    '});',
    '',
    'const lifecycleEndReporter = createLifecycleEndReporter(bridge);',
    'globalThis.__meetingTimelineBridge = bridge;',
    'globalThis.__meetingTimelineOperatorReferenceCapture = bindOperatorReferenceCapture(bridge, lifecycleEndReporter);',
    'globalThis.__meetingTimelineLifecycleEndCapture = bindLifecycleEndCapture(lifecycleEndReporter);',
