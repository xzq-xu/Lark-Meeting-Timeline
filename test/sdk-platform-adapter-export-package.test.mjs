...
assert.equal(google.import_paths.adapter_selection, '@ai-annotation/meeting-timeline-sdk/adapters/platform-adapter-selection');
...
assert.equal(google.import_paths.raw_signal, '@ai-annotation/meeting-timeline-sdk/adapters/platform-raw-signal');
assert.equal(google.commands.adapter_preflight.includes('meeting-platform:adapter-preflight'), true);
assert.equal(google.commands.adapter_selection.includes('meeting-platform:adapter-selection'), true);
assert.equal(google.commands.raw_signal_validation.includes('meeting-platform:raw-signal'), true);
assert.equal(google.commands.raw_signal_validation.includes('--filter-active-speaker-samples=true'), true);
...
assert.equal(google.adapter_selection.ready, true);
...
assert.equal(google.host_files.find((file) => file.source === 'adapter_selection').path, 'google_meet/adapter-selection.json');
assert.equal(google.host_files.find((file) => file.source === 'adapter_blueprint').path, 'google_meet/adapter-blueprint.json');
assert.equal(google.host_files.find((file) => file.source === 'raw_signal_validation').path, 'google_meet/raw-signal-validation.json');
...
assert.equal(google.artifact_refs.adapter_selection.path, 'google_meet/adapter-selection.json');
assert.equal(google.artifact_refs.adapter_blueprint.path, 'google_meet/adapter-blueprint.json');
assert.equal(google.artifact_refs.raw_signal_validation.path, 'google_meet/raw-signal-validation.json');
assert.equal(google.artifacts.runtime_bundle.schema, 'meeting_platform_runtime_bundle');
assert.equal(google.artifacts.adapter_selection.schema, 'meeting_platform_adapter_selection');
assert.equal(google.artifacts.adapter_selection.selection.axis_surface, 'browser_extension');
...
assert.equal(google.artifacts.raw_signal_validation.schema, 'meeting_platform_adapter_raw_signal_validation');
assert.equal(google.artifacts.raw_signal_validation.status, 'ready');
assert.equal(google.artifacts.raw_signal_validation.input_contract.includes('meeting_app_snapshot'), true);
assert.equal(google.artifacts.raw_signal_validation.output_contract.runtime_actions.includes('insert_annotation'), true);
...
assert.equal(custom.host_files.some((file) => file.source === 'raw_signal_validation'), false);
