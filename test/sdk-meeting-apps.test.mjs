...
assert.equal(googleIntegrationPackage.file_paths.includes('runtime-delivery.json'), true);
...
assert.equal(googleIntegrationPackage.entrypoints.platform_runtime_bundle, '@ai-annotation/meeting-timeline-sdk/adapters/platform-runtime-bundle');
assert.equal(googleIntegrationPackage.commands.export_runtime_bundle.includes('meeting-platform:runtime-bundle'), true);
assert.equal(googleIntegrationPackage.runtime_delivery.runtime_bundle_schema, 'meeting_platform_runtime_bundle');
assert.equal(googleIntegrationPackage.runtime_delivery.adapter_route.first_route, 'local_observer_axis');
assert.equal(googleIntegrationPackage.runtime_delivery.readiness.runtime_ready, true);
assert.equal(googleIntegrationPackage.runtime_delivery.host.annotation_timestamp_field, 'captured_at_ms');
...
assert.equal(googleIntegrationPackage.acceptance.production_ready, true);
assert.equal(assertMeetingAppAdapterIntegrationPackage(googleIntegrationPackage, { target: 'production' }).platform, 'google_meet');
...
assert.equal(assertMeetingAppAdapterIntegrationPackageMatrix(integrationPackageMatrix).accepted, true);
assert.throws(
  () => assertMeetingAppAdapterIntegrationPackageMatrix(integrationPackageMatrix, { target: 'production' }),
