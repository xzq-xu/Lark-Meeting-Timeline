...
const tmpDir = await mkdtemp(join(tmpdir(), 'meeting-app-adapter-runtime-config-'));
...
const { stdout } = await execFileAsync(process.execPath, [
  'scripts/meeting-app-adapter-runtime-config.mjs',
...
assert.match(stdout, /meeting_app_adapter_runtime_config_report/);
assert.match(stdout, /ok=yes/);
assert.match(stdout, /configs=4/);
assert.match(stdout, /accepted=4/);
assert.match(stdout, /built_in=2/);
assert.match(stdout, /custom=2/);
assert.match(stdout, /capture_ready=4/);
assert.match(stdout, /mutation_ready=4/);
assert.match(stdout, /content_script_ready=4/);
assert.match(stdout, /written=4/);

const report = JSON.parse(await readFile(reportFile, 'utf8'));
assert.equal(report.type, 'meeting_app_adapter_runtime_config_report');
assert.equal(report.ok, true);
assert.equal(report.config_count, 4);
assert.equal(report.accepted_count, 4);
assert.equal(report.matrix.configs, undefined);
...
assert.equal(whereby.schema, 'meeting_app_adapter_runtime_config');
...
assert.equal(whereby.content_script_options.runtimePreset, false);
assert.equal(whereby.capture_options.controlSelectors.length > 0, true);
...
assert.equal(jsonReport.matrix.configs, undefined);

console.log('ok meeting app adapter runtime config script');
