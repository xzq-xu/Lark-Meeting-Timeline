...
assert.match(stdout, /host_wiring=yes/);
...
assert.equal(report.host_wiring_guide_accepted, true);
assert.equal(report.host_wiring_guide_count, 2);
assert.equal(report.written_files.length >= 43, true);
assert.equal(new Set(report.written_files).size, report.written_files.length);
assert.equal(report.written_files.some((file) => file.endsWith('/extension/src/live-capture.entry.mjs')), true);
...
assert.match(connectorQuickstart, /connector-host-wiring-guide\.json/);
assert.match(connectorQuickstart, /connector-host-wiring-guide-acceptance\.json/);
...
const hostWiringGuide = JSON.parse(await readFile(join(outDir, 'connector-host-wiring-guide.json'), 'utf8'));
assert.equal(hostWiringGuide.schema, 'meeting_app_timeline_connector_host_wiring_guide');
...
const hostWiringGuideAcceptance = JSON.parse(await readFile(join(outDir, 'connector-host-wiring-guide-acceptance.json'), 'utf8'));
