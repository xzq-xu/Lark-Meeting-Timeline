...
assert.equal(report.matrix.runtime_target_message_count, 3);
assert.equal(report.matrix.runtime_open_session_message_count, 3);
...
assert.equal(googleBundle.messaging.bridge_message_types.includes('meeting_timeline.runtime_target'), true);
assert.equal(googleBundle.messaging.bridge_message_types.includes('meeting_timeline.open_session'), true);
assert.equal(googleBundle.messaging.background_message_types.includes('meeting_timeline.preflight_candidates'), true);
assert.equal(googleBundle.messaging.background_message_types.includes('meeting_timeline.runtime_target'), true);
assert.equal(googleBundle.messaging.runtime_bridge_bootstrap.session_plan_kind, 'runtime_target');
assert.equal(googleBundle.messaging.runtime_bridge_bootstrap.target_fields.includes('host_kind'), true);
assert.equal(googleBundle.messaging.examples.runtime_target.type, 'meeting_timeline.runtime_target');
assert.equal(googleBundle.messaging.examples.open_session.type, 'meeting_timeline.open_session');
