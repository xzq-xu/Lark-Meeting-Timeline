...
assert.equal(manifest.sdk_imports.adapter_selection, '@ai-annotation/meeting-timeline-sdk/adapters/platform-adapter-selection');
...
assert.equal(manifest.runtime_contract.adapter_selection_required_before_surface_install, true);
...
assert.equal(manifest.platform_registry.find((row) => row.platform === 'google_meet').adapter_selection_ready, true);
assert.equal(manifest.platform_registry.find((row) => row.platform === 'google_meet').adapter_selection_axis_source, 'local_observer_axis');
assert.equal(manifest.platform_registry.find((row) => row.platform === 'google_meet').adapter_selection_axis_surface, 'browser_extension');
assert.equal(manifest.platform_registry.find((row) => row.platform === 'google_meet').adapter_selection_timestamp_field, 'captured_at_ms');
...
assert.equal(manifest.adapter_selection.enabled, true);
assert.equal(manifest.adapter_selection.platform_count, 2);
assert.equal(manifest.adapter_selection.ready_count, 2);
assert.equal(manifest.adapter_selection.rows.find((row) => row.platform === 'google_meet').axis_surface, 'browser_extension');
assert.equal(manifest.adapter_selection.rows.find((row) => row.platform === 'zoom').axis_surface, 'native_detector');
...
assert.equal(manifest.install_sequence.find((step) => step.id === 'load_adapter_selections').sdk_method, 'platformAdapterSelection');
