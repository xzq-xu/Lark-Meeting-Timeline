...
assert.equal(google.adaptation_strategy.platform, 'google_meet');
assert.equal(google.adaptation_playbook.integration_path.path, 'google_workspace_events_pubsub');
assert.equal(google.adaptation_playbook.next_phase, 'axis_bootstrap');
assert.equal(google.adaptation_playbook.phases.find((phase) => phase.id === 'provider_reconcile').non_blocking_for_realtime, true);
assert.equal(google.adaptation_playbook.risk_profile.provider_axis_risk, 'must_not_block_realtime_axis');
...
assert.equal(google.implementation.kit_methods.includes('platformAdapterSelection'), true);
assert.equal(google.implementation.kit_methods.includes('platformAdapterSelectionMatrix'), true);
...
assert.equal(matrix.adapter_selection_ready_count, 5);
...
assert.equal(matrix.rows.find((row) => row.platform === 'zoom').adapter_selection_axis_surface, 'native_detector');
assert.equal(matrix.rows.find((row) => row.platform === 'webex').adapter_selection_timestamp_field, 'captured_at_ms');
...
assert.equal(matrix.rows.find((row) => row.platform === 'zoom').post_meeting_backfill_supported, true);
