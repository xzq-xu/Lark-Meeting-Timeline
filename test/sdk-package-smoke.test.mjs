...
assert.equal(packedFiles.includes('adapters/platform-adapter-matrix.mjs'), true);
assert.equal(packedFiles.includes('adapters/platform-adapter-matrix.d.ts'), true);
...
assert.equal(packedFiles.includes('adapters/platform-adapter-runtime-recipe.mjs'), true);
assert.equal(packedFiles.includes('adapters/platform-adapter-runtime-recipe.d.ts'), true);
...
const { stdout: adapterRuntimeRecipeBinStdout } = await execFileAsync(
  join(consumerDir, 'node_modules', '.bin', 'meeting-platform-adapter-runtime-recipe'),
  [
    '--platforms=google-meet,zoom',
    '--json=true',
    '--write-recipes=false',
...
const adapterRuntimeRecipeBinReport = JSON.parse(adapterRuntimeRecipeBinStdout);
assert.equal(adapterRuntimeRecipeBinReport.type, 'meeting_platform_adapter_runtime_recipe_report');
assert.equal(adapterRuntimeRecipeBinReport.ok, true);
assert.equal(adapterRuntimeRecipeBinReport.platform_count, 2);
assert.equal(adapterRuntimeRecipeBinReport.runtime_ready_count, 2);
...
const { stdout: adapterRuntimeTargetBinStdout } = await execFileAsync(
  join(consumerDir, 'node_modules', '.bin', 'meeting-platform-adapter-runtime-target'),
  [
    '--platforms=google-meet,zoom',
...
const adapterRuntimeTargetBinReport = JSON.parse(adapterRuntimeTargetBinStdout);
assert.equal(adapterRuntimeTargetBinReport.type, 'meeting_platform_adapter_runtime_target_report');
assert.equal(adapterRuntimeTargetBinReport.ok, true);
assert.equal(adapterRuntimeTargetBinReport.platform, 'google_meet');
assert.equal(adapterRuntimeTargetBinReport.host_kind, 'browser_extension_content_script');
assert.equal(adapterRuntimeTargetBinReport.first_required_method, 'observePlatformCandidates');
...
const adapterDecisionBinReport = JSON.parse(adapterDecisionBinStdout);
assert.equal(adapterDecisionBinReport.type, 'meeting_platform_adapter_decision_report');
assert.equal(adapterDecisionBinReport.ok, true);
assert.equal(adapterDecisionBinReport.platform_count, 2);
assert.equal(adapterDecisionBinReport.host_checklist_ready_count, 2);
...
import {
  ...
  buildMeetingPlatformAdapterMatrix as buildMeetingPlatformAdapterMatrixFromRoot,
  buildMeetingPlatformAdapterMatrixRow as buildMeetingPlatformAdapterMatrixRowFromRoot,
...
} from '@ai-annotation/meeting-timeline-sdk/adapters/platform-adapter-matrix';
import {
...
assert.equal(rootMeetingAppSdk.platformAdapterMatrix({ platforms: ['google-meet', 'zoom'] }).platform_count, 2);
assert.equal(rootMeetingAppSdk.adapterMatrix({ platforms: ['google-meet'] }).rows[0].primary_surface, 'browser_extension');
assert.equal(rootMeetingAppSdk.platformAdapterMatrixRow('zoom').primary_surface, 'native_detector');
assert.equal(rootMeetingAppSdk.adapterMatrixRow('google-meet').realtime_axis.timestamp_field, 'captured_at_ms');
...
assert.equal(rootMeetingAppSdk.platformAdapterRuntimeRecipe({
  url: 'https://meet.google.com/abc-defg-hij',
}).host_wiring.bridge_kind, 'browser_content_script');
assert.equal(rootMeetingAppSdk.adapterRuntimeRecipe({
  platform: 'zoom',
  window: { title: 'Zoom Meeting' },
  process: { name: 'Zoom Workplace' },
}).selected_surface, 'native_detector');
assert.equal(rootMeetingAppSdk.platformAdapterRuntimeRecipeMatrix({}, {
  platforms: ['google-meet', 'zoom'],
}).runtime_ready_count, 2);
assert.equal(rootMeetingAppSdk.assertAdapterRuntimeRecipe({
  url: 'https://meet.google.com/abc-defg-hij',
}).accepted, true);
...
assert.equal(buildMeetingPlatformAdapterMatrixFromRoot({
  platforms: ['google-meet', 'zoom'],
}, {
  baseUrl: 'http://localhost:8787',
}).platform_count, 2);
assert.equal(buildMeetingPlatformAdapterMatrixRowFromRoot('google-meet', {}, {
  baseUrl: 'http://localhost:8787',
}).primary_surface, 'browser_extension');
...
assert.equal(buildMeetingPlatformAdapterRuntimeRecipeFromRoot({
  url: 'https://meet.google.com/abc-defg-hij',
}, {
  baseUrl: 'http://localhost:8787',
}).host_wiring.bridge_kind, 'browser_content_script');
assert.equal(buildMeetingPlatformAdapterRuntimeRecipeMatrixFromRoot({}, {
  baseUrl: 'http://localhost:8787',
  platforms: ['google-meet', 'zoom'],
}).runtime_ready_count, 2);
...
assert.equal(buildMeetingPlatformAdapterMatrix({
  platforms: ['google-meet', 'teams', 'zoom'],
}, {
  baseUrl: 'http://localhost:8787',
}).rows[0].primary_surface, 'browser_extension');
assert.equal(buildMeetingPlatformAdapterMatrixRow('zoom', {}, {
  baseUrl: 'http://localhost:8787',
}).adapter_kind, 'native_first');
...
assert.equal(buildMeetingPlatformAdapterRuntimeRecipe({
  url: 'https://meet.google.com/abc-defg-hij',
}, {
  baseUrl: 'http://localhost:8787',
}).raw_signal_examples.filtered_speaker_event_count, 1);
assert.equal(buildMeetingPlatformAdapterRuntimeRecipeMatrix({}, {
  baseUrl: 'http://localhost:8787',
  platforms: ['zoom'],
...
assert.equal(kit.platformAdapterMatrix({ platforms: ['google-meet', 'zoom'] }).provider_reconcile_required_count, 2);
assert.equal(kit.platformAdapterMatrixRow('google-meet').realtime_axis.provider_events_block_realtime, false);
