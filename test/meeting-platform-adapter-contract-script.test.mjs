...
assert.equal(report.candidate_observer_count, 2);
...
assert.equal(report.rows.find((row) => row.platform === 'google_meet').candidate_observation_ready, true);
assert.equal(report.rows.find((row) => row.platform === 'google_meet').candidate_observer_message_type, 'meeting_timeline.observe_candidates');
assert.equal(report.rows.find((row) => row.platform === 'local_detector').candidate_observation_ready, false);
...
assert.equal(googleContract.candidate_observation.runtime_event_action, 'observe_platform_candidates');
...
assert.match(textStdout, /candidate_observers=1/);
