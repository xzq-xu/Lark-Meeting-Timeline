...
- SDK 回归与安装 smoke：通过；
- 设备无关标准标注生产者：已提供源端时间戳、稳定 ID、重试和失败队列；
- SDK 发布门禁：5/5 平台、15/15 个隔离契约会话通过，共验证 75 条标准生产者标注，不要求设备；
...
以下项目属于平台现场验收，必须由真实外部状态提供，当前没有证据：

- Chrome 尚未加载 `data/meeting-app-extension`；
- 五个平台尚未各完成连续 3 场真实会议；
- 每场 5 条标准生产者人工标注和两位发言人切换尚未采集；
...
SDK 发布标准见 [会议时间轴 SDK 发布验收单](meeting-timeline-sdk-release-acceptance.md)。现场执行标准见 [多会议平台 P0 现场验收单](meeting-platform-p0-field-acceptance.md)。只有现场命令报告 5/5 平台通过，才允许声明“真实平台现场 P0 完成”；这不影响已经独立通过的 SDK 包发布状态。
