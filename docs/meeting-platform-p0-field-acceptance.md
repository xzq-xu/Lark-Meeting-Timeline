...
P0 只验证一条产品主链路：会议软件在本机进入会议后，本地观察器立即建立当前会议轴；电子纸或其他设备产生标注时，SDK 按标注发生时的 `captured_at_ms` 写入当前轴；离会后本地观察器关闭该轴；下一场会议不得继承上一场标注。
...
2. 在会议开始后的不同时间点插入 5 条真实标注。每条在输入源产生时立即写 `captured_at_ms`，不能在服务端收到时补时间。
