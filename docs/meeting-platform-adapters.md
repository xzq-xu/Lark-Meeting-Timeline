...
1. **保留并强化 local detector path**
   ...
   - 对 Google Meet / Teams / Zoom / Lark / Webex 的浏览器 DOM，优先用 `adapters/meeting-app-monitor` 管理轮询和 keep-alive；内部用 `adapters/meeting-app-capture` 采集按钮、participant tile、ariaLabel 和音量/发言状态，再用 `adapters/meeting-apps` preset 归一化。
