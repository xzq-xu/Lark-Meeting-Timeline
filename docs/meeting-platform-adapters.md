...
如果下游需要一个可改造的 host scaffold，而不是自己拼这些模块，用 `platform-host-integration`。生成的 host wrapper 会同时暴露 `runtimeBundles()`、`extensionInstallPlan()`、`handoffBundle()`、`adapterContracts()`、`insertAnnotation()` 和 provider webhook route；`/api/meeting-platform/runtime-bundles` 可作为扩展/WebView/native host 拉取 runtime bundle 的统一入口，`/api/meeting-platform/extension-plan` 可作为扩展构建器读取 manifest、matches 和平台级 content-script bridge 入口的统一入口。
