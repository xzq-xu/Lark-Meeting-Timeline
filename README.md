...
页面的“飞书事件延迟”和事件日志里的 `投递延迟/开会到接收` 用来判断建轴慢的原因：如果这里显示几十秒到几分钟，说明慢在飞书云端投递，不是本机渲染；此时应依赖当前用户会议扫描兜底或端侧/桌面观察器调用 `POST /api/meeting-session/start` 先建轴。
